"""
delete_iris.py — Nettoyage complet du modèle iris-classifier via l'API PredictML
==================================================================================
...
Usage :
  python delete_iris.py                     # dry-run → affiche ce qui sera supprimé
  python delete_iris.py --yes               # supprime sans confirmation
...
if not API_TOKEN:
    ...
    print("    Lancez : API_TOKEN=<votre_token> python delete_iris.py [--yes]")
    sys.exit(1)
...
if predictions_to_delete == 0:
    print("  ✅  Aucune prédiction à supprimer (> 24 h).")
else:
    r = requests.delete(
        f"{API_URL}/predictions/purge",
        headers=HEADERS,
