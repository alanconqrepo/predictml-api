"""
train_cancer_ExtraTrees.py — Breast Cancer — ExtraTreesClassifier (v1.2.0 shadow)
==================================================================================

Interface contract identical to train_cancer.py.
Algorithm: ExtraTreesClassifier (random splits, max_depth=8)
"""
...
rng        = np.random.default_rng(seed=(delta_days + 2) % 1000)
...
try:
    ...
    _csv_filename = f"{MODEL_NAME}_et_{TRAIN_START_DATE}_{TRAIN_END_DATE}_train.csv"
...
if _MLFLOW_AVAILABLE and MLFLOW_TRACKING_URI:
    try:
        ...
        with mlflow.start_run(run_name=f"{MODEL_NAME}_et_{TRAIN_START_DATE}_{TRAIN_END_DATE}") as run:
            mlflow.log_params({"algorithm": "ExtraTrees", **HYPERPARAMS})
            mlflow.log_metrics({"accuracy": acc, "f1_score": f1, "roc_auc": roc_auc})
            mlflow.set_tags({"model_name": MODEL_NAME, "algorithm": "ExtraTrees", "task_type": "binary_classification"})
            try:
