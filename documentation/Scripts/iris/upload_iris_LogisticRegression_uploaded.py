"""
upload_iris_LogisticRegression_uploaded.py — Iris v1.3.0 (LogisticRegression) — uploaded only
====================================================================================================

Upload iris-classifier v1.3.0 with a Pipeline StandardScaler + LogisticRegression.
...
Usage:
  API_TOKEN=<token> python upload_iris_LogisticRegression_uploaded.py
"""
...
DESCRIPTION   = "Pipeline StandardScaler + LogisticRegression iris (v1.3.0) — linear model, C=0.5"
...
TRAIN_SCRIPT_PATH = os.path.join(SCRIPT_DIR, "train_iris_LogisticRegression.py")

if not API_TOKEN:
    print("❌  API_TOKEN not defined.")
    sys.exit(1)
if not os.path.exists(TRAIN_SCRIPT_PATH):
    print(f"❌  train_iris_LogisticRegression.py not found: {TRAIN_SCRIPT_PATH}")
    sys.exit(1)
...
if result.returncode != 0:
    print("❌  train_iris_LogisticRegression.py failed:")
    print(result.stderr)
...
print(f"✅  Training — Acc={acc} F1={f1}" + (f" | MLflow={mlflow_run_id}" if mlflow_run_id else ""))
...
print(f"⏳  Adding tag 'Example' and feature baseline…")
...
if patch.status_code == 200:
    print(f"✅  Tag 'Example' added (is_production=False, no deployment_mode).")
else:
    print(f"⚠️   PATCH failed ({patch.status_code}): {patch.text[:200]}")

print(f"\n   → iris-classifier v{MODEL_VERSION} uploaded but inactive")
