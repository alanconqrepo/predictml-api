"""
delete_iris.py — Full cleanup of the iris-classifier model via the PredictML API
==================================================================================
...
Usage:
  python delete_iris.py                     # dry-run → shows what will be deleted
  python delete_iris.py --yes               # deletes without confirmation
...
and the associated observed_results (ground truth).
"""
...
if not API_TOKEN:
    ...
    print("    Run: API_TOKEN=<your_token> python delete_iris.py [--yes]")
    sys.exit(1)
...
if DRY_RUN:
    ...
    print("=" * 62)
    print(f"  ACTUAL DELETION for model: {MODEL_NAME}")
    print("=" * 62)
...
print(f"\n[3/3] observed_results will be cascade-deleted along with predictions.")
...
print(f"    • {observed_total} observed_result(s) (cascade-deleted)")
print("─" * 62)

if DRY_RUN:
    print()
    print("  Re-run with --yes to execute the deletion:")
    print(f"    python delete_iris.py --yes")
    sys.exit(0)
...
if predictions_to_delete == 0:
    print("  ✅  No predictions to delete (> 24 h).")
else:
    r = requests.delete(
        f"{API_URL}/predictions/purge",
        headers=HEADERS,
    ...
    if r.status_code == 200:
        ...
        if deleted_obs:
            print(f"  ✅  {deleted_obs} observed_result(s) cascade-deleted.")
    else:
