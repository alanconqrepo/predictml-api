...
rng = np.random.default_rng(seed=(delta_days + 17) % 1000)
...
try:
    ...
    _csv_filename = f"{MODEL_NAME}_{TRAIN_START_DATE}_{TRAIN_END_DATE}_ridge_train.csv"
...
if _MLFLOW_AVAILABLE and MLFLOW_TRACKING_URI:
    try:
        ...
        with mlflow.start_run(run_name=f"{MODEL_NAME}_ridge_{TRAIN_START_DATE}") as run:
            mlflow.log_params({"algorithm": "Ridge", **HYPERPARAMS,
                               "scaler": "StandardScaler", "target": TARGET_FEATURE,
            ...
            mlflow.set_tags({"model_name": MODEL_NAME, "algorithm": "Ridge",
                             "task": "regression", "pipeline": "StandardScaler+Ridge"})
            try:
...
output = {
    ...
    "training_dataset": _dataset_minio_path or "scikit-learn Wine dataset — Ridge v1.3.0",
