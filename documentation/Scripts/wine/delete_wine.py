"""
delete_wine.py — Full cleanup of the wine-regressor model via the PredictML API
=================================================================================
...
Usage:
  python delete_wine.py                     # dry-run → shows what will be deleted
  python delete_wine.py --yes               # deletes without confirmation

Environment variables:
  API_URL        API URL               (default: http://localhost:80)
  API_TOKEN      Admin Bearer token — required
...
if not API_TOKEN:
    ...
    print("    Run: API_TOKEN=<your_token> python delete_wine.py [--yes]")
    sys.exit(1)
...
if r.status_code == 200:
    ...
    print(f"      ⚠️  GET /models returned {r.status_code}: {r.text[:120]}")
    versions = []
...
if r.status_code == 200:
    ...
    if oldest_remaining:
        print(f"      Oldest after purge: {oldest_remaining}")
else:
    print(f"      ⚠️  GET purge dry-run returned {r.status_code}: {r.text[:120]}")
