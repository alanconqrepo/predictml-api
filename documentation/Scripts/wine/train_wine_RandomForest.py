"""
train_wine_RandomForest.py — Script de ré-entraînement PredictML — Wine (RandomForest)
========================================================================================

Modèle   : RandomForestRegressor (n_estimators=150, max_depth=8, min_samples_leaf=2)
Dataset  : scikit-learn Wine — régression (cible : teneur en alcool)
Différences vs v1.0.0 GradientBoosting :
  - RandomForest au lieu de GradientBoosting (arbres indépendants, pas de boosting)
  - Entraînement plus rapide, variance réduite
...
print(f"[{MODEL_NAME}] RandomForestRegressor — {TRAIN_START_DATE} → {TRAIN_END_DATE}", file=sys.stderr)
_ts("démarrage")
...
rng = np.random.default_rng(seed=(delta_days + 5) % 1000)
...
try:
    ...
    _csv_filename = f"{MODEL_NAME}_{TRAIN_START_DATE}_{TRAIN_END_DATE}_rf_train.csv"
...
if _MLFLOW_AVAILABLE and MLFLOW_TRACKING_URI:
    try:
        ...
        with mlflow.start_run(run_name=f"{MODEL_NAME}_rf_{TRAIN_START_DATE}") as run:
            mlflow.log_params({"algorithm": "RandomForestRegressor", **HYPERPARAMS,
            ...
            mlflow.set_tags({"model_name": MODEL_NAME, "algorithm": "RandomForestRegressor",
