"""
upload_titanic_ExtraTrees_shadow.py — Titanic v1.2.0 (ExtraTrees) — shadow mode
=================================================================================

Upload titanic-survival v1.2.0 with ExtraTreesClassifier Pipeline in shadow mode.
...
Usage:
  API_TOKEN=<token> python upload_titanic_ExtraTrees_shadow.py
"""
...
DESCRIPTION   = (
    "sklearn Pipeline (OneHotEncoder + StandardScaler + ExtraTreesClassifier) "
    "trained on synthetic Titanic data — v1.2.0 shadow mode, random splits, max_depth=8. "
...
TRAIN_SCRIPT_PATH = os.path.join(SCRIPT_DIR, "train_titanic_ExtraTrees.py")

if not API_TOKEN:
    print("❌  API_TOKEN not defined.")
    sys.exit(1)
if not os.path.exists(TRAIN_SCRIPT_PATH):
    print(f"❌  train_titanic_ExtraTrees.py not found: {TRAIN_SCRIPT_PATH}")
    sys.exit(1)
...
if result.returncode != 0:
    print("❌  train_titanic_ExtraTrees.py failed:")
    print(result.stderr)
    ...
    with open(tmp_pkl.name, "rb") as pkl_fh, open(TRAIN_SCRIPT_PATH, "rb") as train_fh:
        ...
        response = requests.post(
            ...
            files={
                ...
                "train_file": ("train_titanic_ExtraTrees.py", train_fh, "text/plain"),
            },
...
# ── 4. PATCH → shadow + tag "Example" + feature_baseline ─────────────────────

print("⏳  Configuring shadow mode, tag 'Example' and feature baseline…")
...
if patch.status_code == 200:
    print("✅  Shadow mode enabled (is_production=False, deployment_mode=shadow).")
else:
    print(f"⚠️   PATCH shadow failed ({patch.status_code}): {patch.text[:200]}")

print(f"\n   → titanic-survival v{MODEL_VERSION} in shadow mode")
print("   → Predictions silently compared with the production version")
