"""
train_titanic_ExtraTrees.py — PredictML retraining script — Titanic (ExtraTrees)
=================================================================================

Model  : sklearn Pipeline — ColumnTransformer (StandardScaler + OneHotEncoder)
           + ExtraTreesClassifier (n_estimators=150, max_depth=8)
Dataset: synthetic Titanic data — binary classification (survived: 0/1)
Differences vs v1.0.0 GradientBoosting:
  - ExtraTreesClassifier instead of GradientBoostingClassifier
  - Random splits (no optimal threshold search) → faster training
  - bootstrap=False by default for ExtraTrees
...
rng = np.random.default_rng(seed=(delta_days + 7) % 1000)
...
# ── 4. Pipeline — ColumnTransformer + ExtraTrees ─────────────────────────────
...
pipeline = Pipeline([
    ...
    ("classifier",   ExtraTreesClassifier(**HYPERPARAMS)),
])
...
try:
    ...
    _csv_filename = f"{MODEL_NAME}_{TRAIN_START_DATE}_{TRAIN_END_DATE}_extratrees_train.csv"
...
if _MLFLOW_AVAILABLE and MLFLOW_TRACKING_URI:
    try:
        ...
        with mlflow.start_run(run_name=f"{MODEL_NAME}_extratrees_{TRAIN_START_DATE}") as run:
            mlflow.log_params({
                "algorithm": "ExtraTrees",
                "pipeline": "ColumnTransformer(StandardScaler+OneHotEncoder)+ExtraTrees",
            ...
            for _fi_name, _fi_val in feature_importances.items():
                try:
                    ...
                    pass
            mlflow.set_tags({
                "model_name": MODEL_NAME, "algorithm": "ExtraTrees",
