"""
upload_titanic_model.py — Trains and uploads titanic-survival v1.0.0 via the PredictML API
===========================================================================================

This script runs LOCALLY. It:
  1. Executes train_titanic.py in a subprocess to produce the .joblib
  2. Uploads the .joblib + train_titanic.py via POST /models
...
Problem : binary classification (survived: 0/1) with numerical AND categorical features
...
DESCRIPTION = (
    "sklearn Pipeline (OneHotEncoder + StandardScaler + GradientBoostingClassifier) "
    "trained on synthetic data inspired by the Titanic — "
    "predicts passenger survival (0=dead, 1=survived). "
...
RETRAIN_IN_API = False  # True = re-run train.py server-side (using server libraries)
...
if not API_TOKEN:
    ...
    print("    Run: API_TOKEN=<your_token> python upload_titanic_model.py")
    sys.exit(1)

if not os.path.exists(TRAIN_SCRIPT_PATH):
    print(f"❌  train_titanic.py not found: {TRAIN_SCRIPT_PATH}")
    sys.exit(1)
...
# ── 2. Run train_titanic.py ───────────────────────────────────────────────────

tmp_pkl = tempfile.NamedTemporaryFile(suffix=".joblib", delete=False)
tmp_pkl.close()

print(f"⏳  Training via train_titanic.py ({TRAIN_START} → {TRAIN_END})…")
...
try:
    with open(tmp_pkl.name, "rb") as pkl_fh, open(TRAIN_SCRIPT_PATH, "rb") as train_fh:
        ...
        # training_metrics: classification metrics (+ roc_auc for binary)
        _tm = {
            k: round(v, 4)
            for k, v in {
                "accuracy": acc,
        ...
        print(
            f"  [TIMING] POST /models responded in {_upload_elapsed:.2f}s"
...
if response.status_code == 409:
    # Model already exists → PATCH to update
    ...
    if patch_resp.status_code == 200:
        print("✅  ab_test mode and hyperparameters updated.")
    else:
...
# ── 5. Set to production + "Example" tag + feature_baseline ───────────────────
#
# feature_baseline: only the 4 NUMERICAL features (age, fare, parch, sibsp).
# FeatureStats expects mean/std/min/max — categorical features (pclass, sex,
# embarked) are not included in the baseline but remain in
# pipeline.feature_names_in_ for schema validation (/predict).
#
# ─────────────────────────────────────────────────────────────────────────────

print("⏳  Setting to production, 'Example' tag and feature baseline…")
...
# webhook_url: notifies an external system after each prediction (background task).
