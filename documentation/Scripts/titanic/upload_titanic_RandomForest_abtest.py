"""
upload_titanic_RandomForest_abtest.py — Titanic v1.1.0 (RandomForest) — production + ab_test
=============================================================================================

Upload titanic-survival v1.1.0 with RandomForestClassifier Pipeline in A/B test mode.

Deployment:
  - is_production = True
  - deployment_mode = "ab_test" (traffic split with v1.0.0)
  - traffic_weight = 0.5
  - tag "Example"

Usage:
  API_TOKEN=<token> python upload_titanic_RandomForest_abtest.py
"""
...
DESCRIPTION   = (
    "sklearn Pipeline (OneHotEncoder + StandardScaler + RandomForestClassifier) "
    "trained on synthetic Titanic data — v1.1.0 A/B test vs v1.0.0 GradientBoosting. "
...
TRAIN_SCRIPT_PATH = os.path.join(SCRIPT_DIR, "train_titanic_RandomForest.py")

if not API_TOKEN:
    print("❌  API_TOKEN not defined.")
    sys.exit(1)
if not os.path.exists(TRAIN_SCRIPT_PATH):
    print(f"❌  train_titanic_RandomForest.py not found: {TRAIN_SCRIPT_PATH}")
    sys.exit(1)
...
print(f"⏳  Training RandomForest ({TRAIN_START} → {TRAIN_END})…")
...
if result.returncode != 0:
    print("❌  train_titanic_RandomForest.py failed:")
    print(result.stderr)
    ...
    with open(tmp_pkl.name, "rb") as pkl_fh, open(TRAIN_SCRIPT_PATH, "rb") as train_fh:
        ...
        response = requests.post(
            ...
            files={
                ...
                "train_file": ("train_titanic_RandomForest.py", train_fh, "text/plain"),
            },
...
if response.status_code == 409:
    ...
    try:
        ...
        sys.exit(0)
    print("✅  ab_test updated." if patch_resp.status_code == 200
...
# ── 4. PATCH → production + ab_test + tag "Example" + feature_baseline ────────

print("⏳  Setting to production (ab_test), adding tag 'Example' and feature baseline…")
...
if patch.status_code == 200:
    print("✅  Model set to production (ab_test) with tag 'Example' and feature baseline.")
else:
    print(f"⚠️   PATCH failed ({patch.status_code}): {patch.text[:200]}")

print(f"\n   → titanic-survival v{MODEL_VERSION} in A/B test with v1.0.0")
