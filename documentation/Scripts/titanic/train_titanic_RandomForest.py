"""
train_titanic_RandomForest.py — PredictML retraining script — Titanic (RandomForest)
=====================================================================================
...
Differences vs v1.0.0 GradientBoosting:
  - RandomForestClassifier instead of GradientBoostingClassifier
  - Bagging (bootstrap=True) vs boosting
...
print(f"[{MODEL_NAME}] RandomForest — {TRAIN_START_DATE} → {TRAIN_END_DATE}", file=sys.stderr)
_ts("startup")
...
# ── 4. Pipeline — ColumnTransformer + RandomForest ───────────────────────────
    ...
    try:
        ...
        with mlflow.start_run(run_name=f"{MODEL_NAME}_rf_{TRAIN_START_DATE}") as run:
            mlflow.log_params({
                ...
                "pipeline": "ColumnTransformer(StandardScaler+OneHotEncoder)+RF",
            ...
            for _fi_name, _fi_val in feature_importances.items():
                try:
                    ...
                    pass
            mlflow.set_tags({
                "model_name": MODEL_NAME, "algorithm": "RandomForest",
...
output = {
    ...
    "classes":            ["0", "1"],
    "hyperparameters": {
        "pipeline": "ColumnTransformer(StandardScaler+OneHotEncoder)+RandomForest",
        **HYPERPARAMS,
    },
    "training_dataset": (
        _dataset_minio_path
