            ...
            if not existing:
                db.add(
                    ModelMetadata(
                        ...
                        version=ver,
                        minio_bucket="models",
                        minio_object_key=f"{MODEL_CLF}/v{ver}.pkl",
                        classes=[0, 1],
                        is_active=True,
        ...
        if not existing:
            db.add(
                ModelMetadata(
                    ...
                    version="1.0.0",
                    minio_bucket="models",
                    minio_object_key=f"{MODEL_REG}/v1.0.0.pkl",
                    classes=None,
                    is_active=True,
                    is_production=True,
                )
        ...
        if not existing:
            db.add(
                ModelMetadata(
                    ...
                    version="1.0.0",
                    minio_bucket="models",
                    minio_object_key=f"{MODEL_NO_OBS}/v1.0.0.pkl",
                    classes=[0, 1],
                    is_active=True,
                    is_production=True,
                )
            ...
            if not existing:
                db.add(
                    ModelMetadata(
                        ...
                        version=ver,
                        minio_bucket="models",
                        minio_object_key=f"{MODEL_MULTI_VER}/v{ver}.pkl",
                        classes=[0, 1],
                        is_active=True,
        ...
        if not existing:
            ...
            db.add(
                ModelMetadata(
                    ...
                    version="1.0.0",
                    minio_bucket="models",
                    minio_object_key=f"{MODEL_TRAINING_STATS}/v1.0.0.pkl",
                    classes=[0, 1],
                    is_active=True,
                    is_production=True,
                    training_stats={
